...
export function simulateMetricsTick(
  ...
  return {
    ...
    calls_answered: Math.max(attempted - failed, 0),
    calls_completed: completed,
    calls_failed: failed,
    asr: attempted > 0 ? Math.round((Math.max(attempted - failed, 0) / attempted) * 1000) / 10 : 0,
    csr: attempted > 0 ? Math.round((completed / attempted) * 1000) / 10 : 0,
