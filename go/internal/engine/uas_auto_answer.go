...
func (u *UasAutoAnswer) handleCall(ctx context.Context, ag *agent.ExtensionAgent, rawInvite string) {
	...
	if cfg.MediaEnabled {
		...
		rtpEP, err = rtp.NewRtpEndpointFullCodec(
			ag.LocalHost(),
			cfg.RTPPtime,
			rtpSourceCodec(negotiatedCodec, cfg.RTPCodec),
			cfg.IsQoSEnabled(),
			cfg.IsRTCPSREnabled(),
	...
	result := CallResult{
		...
		UASControllerHost:   ag.ControllerHost,
		UASControllerPort:   ag.ControllerPort,
		UASAgentGroupID:     ag.AgentGroupID,
		UASZoneID:           ag.ZoneID,
		AgentGroupID:        ag.AgentGroupID,
...
func (u *UasAutoAnswer) handleTimeout(
	...
	result := CallResult{
		...
		UASZoneID:             ag.ZoneID,
		AgentGroupID:          ag.AgentGroupID,
		FailureControllerRole: "uas",
		FailureControllerHost: ag.ControllerHost,
		FailureControllerPort: ag.ControllerPort,
...
func rtpSourceCodec(negotiatedCodec, configuredCodec string) string {
	if rtp.IsG729PreEncodedAudio(configuredCodec) && rtp.ProfileForCodec(negotiatedCodec).Name == "G729" {
